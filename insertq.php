...
if ($stmt->num_rows > 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO quejas (id_usuario, descripcion, tipos) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $id_usuario, $descripcion, $feedbackType);

    if ($stmt->execute()) {
        $response['message'] .= " Feedback guardado exitosamente para el usuario ID: $id_usuario.";
    } else {
        $response['success'] = false;
